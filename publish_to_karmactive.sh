...
# Usage: WP_USER="username" WP_PASS="app-password" bash publish_to_karmactive.sh
...
if [[ -z "$WP_USER" || -z "$WP_PASS" ]]; then
  echo "ERROR: Set WP_USER and WP_PASS environment variables first."
  echo "Usage: WP_USER='your_user' WP_PASS='your_app_password' bash publish_to_karmactive.sh"
  exit 1
fi

AUTH=$(echo -n "$WP_USER:$WP_PASS" | base64)

# Helper: get or create a tag by slug, return its ID
get_or_create_tag() {
  local slug="$1"
  local name="$2"
  # Try to find existing
  local result=$(curl -s -H "Authorization: Basic $AUTH" \
    "${WP_BASE}/tags?slug=${slug}&per_page=1")
  local id=$(echo "$result" | python3 -c "import sys,json; data=json.load(sys.stdin); print(data[0]['id'] if data else '')" 2>/dev/null)
  if [[ -z "$id" ]]; then
    # Create it
    local create=$(curl -s -X POST -H "Authorization: Basic $AUTH" \
      -H "Content-Type: application/json" \
      -d "{\"name\":\"${name}\",\"slug\":\"${slug}\"}" \
      "${WP_BASE}/tags")
    id=$(echo "$create" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('id',''))" 2>/dev/null)
  fi
  echo "$id"
}
...
get_category_id() {
  local slug="$1"
  local result=$(curl -s -H "Authorization: Basic $AUTH" \
    "${WP_BASE}/categories?slug=${slug}&per_page=1")
  echo "$result" | python3 -c "import sys,json; data=json.load(sys.stdin); print(data[0]['id'] if data else '')" 2>/dev/null
}
...
publish_post() {
  ...
  local slug="$2"
  local content="$3"
  local cat_ids="$4"       # JSON array string e.g. "[12,34]"
  local tag_ids="$5"       # JSON array string e.g. "[56,78]"
  local yoast_title="$6"
  local yoast_desc="$7"
  local yoast_focuskw="$8"
...
data = {
  ...
  'slug': sys.argv[2],
  'content': sys.argv[3],
  'categories': json.loads(sys.argv[4]),
  'tags': json.loads(sys.argv[5]),
  'status': 'publish',
  'meta': {
    '_yoast_wpseo_title': sys.argv[6],
    '_yoast_wpseo_metadesc': sys.argv[7],
    '_yoast_wpseo_focuskw': sys.argv[8],
  }
}
print(json.dumps(data))
" "$title" "$slug" "$content" "$cat_ids" "$tag_ids" "$yoast_title" "$yoast_desc" "$yoast_focuskw")
  ...
  local post_id=$(echo "$response" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('id','ERROR: '+str(d.get('message','unknown'))))" 2>/dev/null)
  local post_link=$(echo "$response" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('link',''))" 2>/dev/null)
  echo "  → Published: ID=$post_id | URL=$post_link"
}

echo "=============================="
echo " Karmactive.com Article Publisher"
echo "=============================="
echo "Testing API connection..."
WHOAMI=$(curl -s -H "Authorization: Basic $AUTH" "${WP_BASE}/users/me" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('name','AUTH FAILED: '+str(d.get('message',''))))" 2>/dev/null)
echo "Authenticated as: $WHOAMI"
echo ""

# ── Pre-fetch category IDs ──────────────────────────────────────────────
echo "Fetching category IDs..."
CAT_AVIATION=$(get_category_id "aviation")
CAT_NEWS=$(get_category_id "news")
CAT_HEALTH=$(get_category_id "health")
CAT_ENTERTAINMENT=$(get_category_id "entertainment")
CAT_ASTRONOMY=$(get_category_id "astronomy")
CAT_SCIENCE=$(get_category_id "science")
CAT_UK=$(get_category_id "uk")
CAT_DISCOVERY=$(get_category_id "discovery")
CAT_MARINE=$(get_category_id "marine-life")
CAT_BUSINESS=$(get_category_id "business")
CAT_FOOD=$(get_category_id "food-drinks")
CAT_WATER=$(get_category_id "water")
CAT_CLIMATE=$(get_category_id "climate")
CAT_EUROPE=$(get_category_id "europe")
CAT_ADVENTURE=$(get_category_id "adventure")
CAT_WEATHER=$(get_category_id "weather")
CAT_CANADA=$(get_category_id "canada")
CAT_POLICY=$(get_category_id "policy")
CAT_POLITICS=$(get_category_id "politics")
CAT_DISASTER=$(get_category_id "disaster")
CAT_USA=$(get_category_id "usa")
CAT_ENERGY=$(get_category_id "energy")
CAT_WORLD=$(get_category_id "world")
CAT_TECHNOLOGY=$(get_category_id "technology")
CAT_AI=$(get_category_id "artificial-intelligence")
echo "Done."
echo ""

# ── Pre-fetch/create tag IDs ────────────────────────────────────────────
echo "Fetching tag IDs..."
TAG_AVIATION_SAFETY=$(get_or_create_tag "aviation-safety" "Aviation Safety")
TAG_AIRLINES=$(get_or_create_tag "airlines" "Airlines")
TAG_AVIATION=$(get_or_create_tag "aviation" "Aviation")
TAG_PASSENGERS=$(get_or_create_tag "passengers" "Passengers")
TAG_HEALTH=$(get_or_create_tag "health" "Health")
TAG_CELEBRITY=$(get_or_create_tag "celebrity" "Celebrity")
TAG_ENTERTAINMENT=$(get_or_create_tag "entertainment" "Entertainment")
TAG_NEUROSCIENCE=$(get_or_create_tag "neuroscience" "Neuroscience")
TAG_ASTRONOMY=$(get_or_create_tag "astronomy" "Astronomy")
TAG_SCIENCE=$(get_or_create_tag "science" "Science")
TAG_TELESCOPE=$(get_or_create_tag "telescope" "Telescope")
TAG_BIODIVERSITY=$(get_or_create_tag "biodiversity" "Biodiversity")
TAG_MARINE_LIFE=$(get_or_create_tag "marine-life" "Marine Life")
TAG_MARINE_BIODIVERSITY=$(get_or_create_tag "marine-biodiversity" "Marine Biodiversity")
TAG_OCEAN_CONSERVATION=$(get_or_create_tag "ocean-conservation" "Ocean Conservation")
TAG_NHS=$(get_or_create_tag "nhs" "NHS")
TAG_HEALTHCARE=$(get_or_create_tag "healthcare" "Healthcare")
TAG_MENTAL_HEALTH=$(get_or_create_tag "mental-health" "Mental Health")
TAG_WORKPLACE=$(get_or_create_tag "workplace" "Workplace")
TAG_CONSTRUCTION=$(get_or_create_tag "construction" "Construction")
TAG_BANKRUPTCY=$(get_or_create_tag "bankruptcy" "Bankruptcy")
TAG_ECONOMICS=$(get_or_create_tag "economics" "Economics")
TAG_FOOD_SAFETY=$(get_or_create_tag "food-safety" "Food Safety")
TAG_FOOD_RECALL=$(get_or_create_tag "food-recall" "Food Recall")
TAG_ALLERGY=$(get_or_create_tag "allergy" "Allergy")
TAG_ALLERGY_ALERT=$(get_or_create_tag "allergy-alert" "Allergy Alert")
TAG_WATER=$(get_or_create_tag "water" "Water")
TAG_BEACH=$(get_or_create_tag "beach" "Beach")
TAG_WILDFIRES=$(get_or_create_tag "wildfires" "Wildfires")
TAG_CLIMATE=$(get_or_create_tag "climate" "Climate")
TAG_NATURAL_DISASTER=$(get_or_create_tag "natural-disaster" "Natural Disaster")
TAG_SPAIN=$(get_or_create_tag "spain" "Spain")
TAG_HIKING=$(get_or_create_tag "hiking" "Hiking")
TAG_MOUNTAIN=$(get_or_create_tag "mountain" "Mountain")
TAG_SEVERE_WEATHER=$(get_or_create_tag "severe-weather" "Severe Weather")
TAG_STORM=$(get_or_create_tag "storm" "Storm")
TAG_EXTREME_WEATHER=$(get_or_create_tag "extreme-weather" "Extreme Weather")
TAG_CANADA=$(get_or_create_tag "canada" "Canada")
TAG_BANK_OF_CANADA=$(get_or_create_tag "bank-of-canada" "Bank of Canada")
TAG_INTEREST_RATES=$(get_or_create_tag "interest-rates" "Interest Rates")
TAG_ECONOMIC_GROWTH=$(get_or_create_tag "economic-growth" "Economic Growth")
TAG_UNITED_NATIONS=$(get_or_create_tag "united-nations" "United Nations")
TAG_HUMAN_RIGHTS=$(get_or_create_tag "human-rights" "Human Rights")
TAG_HEALTHCARE_WORKERS=$(get_or_create_tag "healthcare-workers" "Healthcare Workers")
TAG_WINTER_STORM=$(get_or_create_tag "winter-storm" "Winter Storm")
TAG_OIL=$(get_or_create_tag "oil" "Oil")
TAG_ENERGY=$(get_or_create_tag "energy" "Energy")
TAG_MIDDLE_EAST=$(get_or_create_tag "middle-east" "Middle East")
TAG_IRAN=$(get_or_create_tag "iran" "Iran")
TAG_AI_TECHNOLOGY=$(get_or_create_tag "ai-technology" "AI Technology")
TAG_SEMICONDUCTOR=$(get_or_create_tag "semiconductor" "Semiconductor")
TAG_STOCK_MARKET=$(get_or_create_tag "stock-market" "Stock Market")
TAG_TECHNOLOGY=$(get_or_create_tag "technology" "Technology")
echo "Done."
echo ""
...
publish_post \
  "Ryanair Passenger Nearly Sucked From Window Receives Only Rebooking Offer - No Compensation" \
  "ryanair-window-incident-passenger-compensation-2026" \
  "$CONTENT_1" \
  "[${CAT_AVIATION},${CAT_NEWS}]" \
  "[${TAG_AVIATION_SAFETY},${TAG_AIRLINES},${TAG_AVIATION},${TAG_PASSENGERS}]" \
  "Ryanair Passenger Nearly Sucked From Window Receives Only Rebooking Offer - No Compensation" \
  "Ryanair passenger Ljubisa Karovic was nearly sucked through a window at 15,000 feet. He received only a rebooking offer, no compensation. Greek lawyer pursuing significant compensation. EU261 violations alleged." \
  "Ryanair passenger compensation incident"

# ══════════════════════════════════════════════════════════════════
# STORY 2: CARLY SIMON PARKINSON'S
# ══════════════════════════════════════════════════════════════════
echo "[2/17] Publishing: Carly Simon Parkinson's..."
CONTENT_2='<p>Music legend Carly Simon has disclosed she has been diagnosed with <a href="https://www.nia.nih.gov/health/parkinsons-disease/parkinsons-disease-causes-symptoms-and-treatments">Parkinson&#8217;s disease</a>, a progressive neurological condition that has affected her ability to perform publicly. On July 27, 2026, the 83-year-old icon shared her diagnosis through a statement to major news outlets, explaining that she has spent considerable time learning to manage the disorder before deciding to discuss it publicly.</p>
...
publish_post \
  ...
  "carly-simon-parkinsons-disease-2026" \
  "$CONTENT_2" \
  "[${CAT_HEALTH},${CAT_ENTERTAINMENT}]" \
  "[${TAG_HEALTH},${TAG_CELEBRITY},${TAG_ENTERTAINMENT},${TAG_NEUROSCIENCE}]" \
  "Carly Simon Reveals Parkinson's Disease Diagnosis at 83 - Shares Health Journey" \
  "Carly Simon, 83, reveals Parkinson's disease diagnosis in July 2026. Legend discusses treatment with dopamine therapies and her ongoing health journey. Also treated for skin cancer basal cell carcinoma." \
  "Carly Simon Parkinson's diagnosis"

# ══════════════════════════════════════════════════════════════════
# STORY 3: JODRELL BANK OBSERVATORY
# ══════════════════════════════════════════════════════════════════
echo "[3/17] Publishing: Jodrell Bank Observatory..."
CONTENT_3='<p>Jodrell Bank Observatory, a <a href="https://whc.unesco.org/en/list/1306/">UNESCO World Heritage Site</a> and landmark of British astronomy research, is at risk of closure by April 1, 2028 when a major funding source ends, threatening thousands of researchers and critical space exploration work. <a href="https://www.ukri.org/">UK Research and Innovation (UKRI)</a> announced it will stop funding e-MERLIN, the radio telescope network that forms the core of Jodrell Bank&#8217;s operations, when the current funding agreement expires in March 2028.</p>
...
<p>Scientists rely on Jodrell Bank for cutting-edge research into black holes, neutron stars, gravitational waves, and deep space phenomena. The potential loss of this capability represents a significant step backward for British science and international collaborative research. The coming 20 months are critical for securing alternative funding.</p>'

publish_post \
  "Jodrell Bank Observatory Faces 2028 Closure Over Funding Cuts - Scientists Call It Devastating" \
  "jodrell-bank-observatory-closure-2028-funding" \
  "$CONTENT_3" \
  "[${CAT_ASTRONOMY},${CAT_SCIENCE},${CAT_UK}]" \
  "[${TAG_ASTRONOMY},${TAG_SCIENCE},${TAG_TELESCOPE}]" \
  "Jodrell Bank Observatory Faces 2028 Closure Over Funding Cuts" \
  "Jodrell Bank Observatory faces April 2028 closure as UKRI withdraws £2.8M funding. UNESCO World Heritage site affects 3,000 researchers. Royal Astronomical Society calls decision 'devastating' for UK science." \
  "Jodrell Bank funding closure"

# ══════════════════════════════════════════════════════════════════
# STORY 4: DEEP SEA DISCOVERY
# ══════════════════════════════════════════════════════════════════
echo "[4/17] Publishing: Deep Sea Discovery..."
CONTENT_4='<p>Marine biologists have documented an unprecedented number of previously unknown species living in Earth&#8217;s deepest ocean zones, marking a significant milestone in understanding life in extreme environments and highlighting how much remains unknown about our planet&#8217;s <a href="https://www.karmactive.com/category/nature/marine-life/">marine biodiversity</a>.</p>
...
publish_post \
  "Scientists Discover 1,121 New Ocean Species in 2026 - Deep Sea Biodiversity Crisis" \
  "deep-sea-1121-new-species-2026" \
  "$CONTENT_4" \
  "[${CAT_DISCOVERY},${CAT_MARINE},${CAT_SCIENCE}]" \
  "[${TAG_BIODIVERSITY},${TAG_MARINE_LIFE},${TAG_MARINE_BIODIVERSITY},${TAG_OCEAN_CONSERVATION}]" \
  "Scientists Discover 1,121 New Ocean Species in 2026 - Deep Sea Biodiversity Crisis" \
  "Scientists discovered 1,121 new marine species in 2026, including 24 new amphipods. Deep-sea mining threatens to destroy ecosystems before we understand them, reducing populations by 37% in test zones." \
  "Deep sea species biodiversity"

# ══════════════════════════════════════════════════════════════════
# STORY 5: MERSEY CARE NHS TRUST
# ══════════════════════════════════════════════════════════════════
echo "[5/17] Publishing: Mersey Care NHS Trust..."
CONTENT_5='<p>A confidential <a href="https://www.nhsemployers.org/">NHS report</a> has uncovered a deeply problematic workplace environment at Mersey Care NHS Foundation Trust, one of England&#8217;s largest mental health and community care providers, revealing widespread staff anxiety, fear, and leadership dysfunction.</p>
...
publish_post \
  "Toxic Culture Found at Mersey Care NHS Trust - Staff Fear Speaking Up" \
  "mersey-care-nhs-trust-toxic-culture-2026" \
  "$CONTENT_5" \
  "[${CAT_HEALTH},${CAT_UK}]" \
  "[${TAG_NHS},${TAG_HEALTHCARE},${TAG_MENTAL_HEALTH},${TAG_WORKPLACE}]" \
  "Toxic Culture Found at Mersey Care NHS Trust - Staff Fear Speaking Up" \
  "NHS investigation finds toxic workplace culture at Mersey Care Trust. Staff report fear, anxiety, favoritism, nepotism. Bullying costs NHS £600M annually in lost productivity. Trust implements reform initiatives." \
  "NHS trust toxic workplace"

# ══════════════════════════════════════════════════════════════════
# STORY 6: UK CONSTRUCTION COLLAPSE
# ══════════════════════════════════════════════════════════════════
echo "[6/17] Publishing: UK Construction Collapse..."
CONTENT_6='<p>The British <a href="https://www.karmactive.com/category/business/">construction industry</a> continued its precarious state in 2026 with several significant firms entering administration, highlighting ongoing structural challenges within a sector that remains one of the country&#8217;s most financially fragile.</p>
...
publish_post \
  ...
  "uk-construction-firms-collapse-2026-administration" \
  "$CONTENT_6" \
  "[${CAT_BUSINESS},${CAT_UK}]" \
  "[${TAG_CONSTRUCTION},${TAG_BANKRUPTCY},${TAG_ECONOMICS}]" \
  "UK Construction Sector Crisis: Major Firms Collapse in 2026 Administration Wave" \
  "UK construction sector faces crisis with multiple administrations in 2026. Ardmore Group, Zentia, Agetur all failing. Construction represents 10% of all administrations. Rising costs and weak demand cited." \
  "UK construction sector collapse"

# ══════════════════════════════════════════════════════════════════
# STORY 7: CHOCOLATE RECALL
# ══════════════════════════════════════════════════════════════════
echo "[7/17] Publishing: Chocolate Recall..."
CONTENT_7='<p>British consumers have been warned not to consume several chocolate products due to undeclared allergens that pose serious health risks for people with milk and soya allergies, prompting urgent recall actions across major retailers.</p>
...
publish_post \
  "Chocolate Recalled Across Britain Over Undeclared Milk and Soya Allergens" \
  "chocolate-recall-undeclared-allergens-2026" \
  "$CONTENT_7" \
  "[${CAT_FOOD},${CAT_UK},${CAT_HEALTH}]" \
  "[${TAG_FOOD_SAFETY},${TAG_FOOD_RECALL},${TAG_ALLERGY},${TAG_ALLERGY_ALERT}]" \
  "Chocolate Recalled Across Britain Over Undeclared Milk and Soya Allergens" \
  "Chocolate products recalled in UK for undeclared milk, soya allergens. Bon Bons, Millennium, Seggiano products affected. 6% of adults have food allergies. FSA urges consumers not to eat affected products." \
  "Chocolate allergen recall UK"

# ══════════════════════════════════════════════════════════════════
# STORY 8: BEACH DROWNING
# ══════════════════════════════════════════════════════════════════
echo "[8/17] Publishing: Beach Drowning..."
CONTENT_8='<p>A father and daughter died after being pulled from the sea at Fistral Beach in Newquay, Cornwall on July 26, marking a tragic incident during what should have been a holiday for the family.</p>
...
publish_post \
  "Father and Daughter Die After Being Pulled From Cornish Beach - Fistral Beach Drowning" \
  "fistral-beach-newquay-drowning-2026" \
  "$CONTENT_8" \
  "[${CAT_WATER},${CAT_UK},${CAT_NEWS}]" \
  "[${TAG_WATER},${TAG_BEACH},${TAG_SEVERE_WEATHER}]" \
  "Father and Daughter Die After Being Pulled From Cornish Beach" \
  "Father and daughter die after being pulled from Fistral Beach, Newquay. Residents from Doncaster. Lifeguards prevent 80% more drowning incidents. Water safety experts urge tide checks and supervision." \
  "Fistral Beach drowning water"

# ══════════════════════════════════════════════════════════════════
# STORY 9: EUROPE WILDFIRES
# ══════════════════════════════════════════════════════════════════
echo "[9/17] Publishing: Europe Wildfires..."
CONTENT_9='<p>Record-setting wildfires have swept through France and Spain in July 2026, forcing approximately 330,000 people to evacuate their homes as multiple fires spread rapidly across the region during an intensifying heat wave.</p>
...
publish_post \
  "European Wildfires Force 330,000 Evacuations - Spain's Largest Fire on Record" \
  "europe-wildfires-france-spain-2026-evacuations" \
  "$CONTENT_9" \
  "[${CAT_CLIMATE},${CAT_EUROPE},${CAT_DISASTER}]" \
  "[${TAG_WILDFIRES},${TAG_CLIMATE},${TAG_NATURAL_DISASTER},${TAG_SPAIN}]" \
  "European Wildfires Force 330,000 Evacuations - Spain's Largest Fire on Record" \
  "Record wildfires in France, Spain force 330,000 evacuations. Spain's largest fire on record. France 3.4x average fires; Spain 1.7x average. Heat waves dry landscape progressively. At least 1 death reported." \
  "Europe wildfires heat evacuations"

# ══════════════════════════════════════════════════════════════════
# STORY 10: MONTANA HIKING ACCIDENT
# ══════════════════════════════════════════════════════════════════
echo "[10/17] Publishing: Montana Hiking Accident..."
CONTENT_10='<p>A Montana hiker demonstrated remarkable resilience when he walked more than 10 miles for help after accidentally impaling himself on his own trekking pole during a hiking accident on July 20, 2026.</p>
...
publish_post \
  "Hiker Walks 10 Miles With Trekking Pole Impaled Through Chest - Survives Montana Accident" \
  "montana-hiker-impaled-trekking-pole-2026" \
  "$CONTENT_10" \
  "[${CAT_ADVENTURE},${CAT_NEWS}]" \
  "[${TAG_HIKING},${TAG_MOUNTAIN},${TAG_HEALTH}]" \
  "Hiker Walks 10 Miles With Trekking Pole Impaled Through Chest" \
  "David Cifaldi, 32, impaled himself on trekking pole while hiking Montana's tallest mountain. Walked 10+ miles to safety with pole still embedded. Survived with injuries. Plans to return to hiking." \
  "Hiking pole impalement survival"

# ══════════════════════════════════════════════════════════════════
# STORY 11: EDMONTON THUNDERSTORM
# ══════════════════════════════════════════════════════════════════
echo "[11/17] Publishing: Edmonton Thunderstorm..."
CONTENT_11='<p>A powerful thunderstorm blasted through Edmonton in late July 2026, leaving behind downed trees, road closures, and widespread power outages affecting thousands of residents across the city&#8217;s northern neighborhoods.</p>
...
publish_post \
  "Severe Thunderstorm Hits Edmonton - 14,000 Without Power After July Storm" \
  "edmonton-thunderstorm-power-outage-2026" \
  "$CONTENT_11" \
  "[${CAT_WEATHER},${CAT_CANADA}]" \
  "[${TAG_SEVERE_WEATHER},${TAG_STORM},${TAG_EXTREME_WEATHER},${TAG_CANADA}]" \
  "Severe Thunderstorm Hits Edmonton - 14,000 Without Power After July Storm" \
  "Severe thunderstorm hits Edmonton July 25-26. Nearly 14,000 customers lost power. Downed trees, road closures in northern neighborhoods. Tornado warning issued for Yellowhead County. Crews working overnight." \
  "Edmonton thunderstorm power outage"

# ══════════════════════════════════════════════════════════════════
# STORY 12: BANK OF CANADA RATES
# ══════════════════════════════════════════════════════════════════
echo "[12/17] Publishing: Bank of Canada Rates..."
CONTENT_12='<p>The <a href="https://www.bankofcanada.ca/">Bank of Canada maintained its target overnight interest rate at 2.25%</a> during its July 15, 2026 policy announcement, marking the sixth consecutive decision to hold rates steady as economic indicators show signs of improving growth.</p>
...
<p>Canada&#8217;s inflation situation continues to improve according to bank projections. The bank expects inflation to remain slightly elevated in June before beginning to decline more noticeably in coming months. Central bank forecasts predict inflation will fall to 2.5 percent during the second half of 2026, before reaching the bank&#8217;s two percent target in early 2027.</p>
...
publish_post \
  "Bank of Canada Holds Interest Rate at 2.25% - Signals Economic Recovery" \
  "bank-of-canada-interest-rate-2026-hold" \
  "$CONTENT_12" \
  "[${CAT_POLICY},${CAT_CANADA}]" \
  "[${TAG_BANK_OF_CANADA},${TAG_INTEREST_RATES},${TAG_ECONOMIC_GROWTH},${TAG_ECONOMICS}]" \
  "Bank of Canada Holds Interest Rate at 2.25% - Signals Economic Recovery" \
  "Bank of Canada holds interest rate at 2.25% for sixth consecutive time. Economic growth resuming after stalling. Inflation expected to fall to 2.5% H2 2026, reach 2% target in early 2027." \
  "Bank Canada interest rate"

# ══════════════════════════════════════════════════════════════════
# STORY 13: US-FRANCE UN WALKOUT
# ══════════════════════════════════════════════════════════════════
echo "[13/17] Publishing: US-France UN Walkout..."
CONTENT_13='<p>The United States staged a diplomatic walkout from a UN Security Council meeting on July 28, protesting French remarks that compared America to North Korea and Russia regarding human rights positions, escalating tensions between the longtime allies.</p>
...
<p><a href="https://www.un.org/">Dan Negrea, an alternate representative to the General Assembly and senior US delegation official</a>, responded angrily to the French criticism, dismissing their remarks as &#8220;disingenuous grandstanding&#8221; and objecting to being grouped with authoritarian nations on human rights matters.</p>
...
publish_post \
  "US Walks Out on France at UN Over Human Rights Criticism - Diplomatic Rift Deepens" \
  "us-france-un-walkout-2026-human-rights" \
  "$CONTENT_13" \
  "[${CAT_POLITICS},${CAT_POLICY}]" \
  "[${TAG_UNITED_NATIONS},${TAG_HUMAN_RIGHTS},${TAG_ECONOMICS}]" \
  "US Walks Out on France at UN Over Human Rights Criticism" \
  "US delegation walks out of UN Security Council during France's remarks. Tensions over US vote with North Korea, Russia against UN HR Commissioner extension. France criticized US human rights record." \
  "US France UN diplomacy"

# ══════════════════════════════════════════════════════════════════
# STORY 14: CANADIAN ED PHYSICIAN CRISIS
# ══════════════════════════════════════════════════════════════════
echo "[14/17] Publishing: Canadian ED Physician Crisis..."
CONTENT_14='<p>A national survey of Canadian emergency medicine physicians has revealed a profession in crisis, with nearly half reducing work hours and one in ten leaving the specialty entirely due to burnout and poor working conditions.</p>
...
publish_post \
  "Canadian Emergency Doctors Leaving Profession - 10% Exit Specialty Due to Burnout" \
  "canadian-ed-physician-crisis-burnout-2026" \
  "$CONTENT_14" \
  "[${CAT_HEALTH},${CAT_CANADA}]" \
  "[${TAG_HEALTHCARE},${TAG_HEALTHCARE_WORKERS},${TAG_MENTAL_HEALTH},${TAG_CANADA}]" \
  "Canadian Emergency Doctors Leaving Profession - 10% Exit Specialty Due to Burnout" \
  "Survey: 10% Canadian emergency physicians leaving specialty, 50% reducing hours. Burnout unchanged since COVID peak. Patient wait times 14+ hours common. CIHI reports wait times increasing annually." \
  "ED physician burnout crisis"

# ══════════════════════════════════════════════════════════════════
# STORY 15: ALASKA WINTER STORM
# ══════════════════════════════════════════════════════════════════
echo "[15/17] Publishing: Alaska Winter Storm..."
CONTENT_15='<p>The National Weather Service issued a rare midsummer snow warning for Alaska, cautioning residents and travelers about unexpected winter conditions developing in late July 2026. A Winter Weather Advisory has been issued for the Central Brooks Range, including the Dalton Highway, with expected snow accumulations between 2 and 6 inches.</p>
...
<p><a href="https://www.karmactive.com/category/science/">Summer snow in Alaska&#8217;s high-altitude areas results from the region&#8217;s proximity to Arctic air masses</a> and the elevation-related cooling effects that allow snow to fall even during summer months when lower elevations experience typical warm weather. The event, while noteworthy, demonstrates Alaska&#8217;s extreme climate variability and the region&#8217;s exposure to rapid weather changes across seasons.</p>'

publish_post \
  "Rare July Snow Warning Issued for Alaska's Central Brooks Range - Winter in Summer" \
  "alaska-summer-snow-warning-2026" \
  "$CONTENT_15" \
  "[${CAT_WEATHER},${CAT_USA}]" \
  "[${TAG_EXTREME_WEATHER},${TAG_WINTER_STORM},${TAG_SEVERE_WEATHER},${TAG_CLIMATE}]" \
  "Rare July Snow Warning Issued for Alaska's Central Brooks Range" \
  "Winter Weather Advisory for Alaska's Central Brooks Range. 2-6 inches snow expected in late July. Upper 30s-40s°F highs. Dalton Highway travel hazardous. Rare but not unprecedented summer snow event." \
  "Alaska July snow storm"

# ══════════════════════════════════════════════════════════════════
# STORY 16: OIL PRICE DROP
# ══════════════════════════════════════════════════════════════════
echo "[16/17] Publishing: Oil Price Drop..."
CONTENT_16='<p>International oil prices dropped more than 5 percent on July 27&#8211;28 following announcements that the United States and Iran had paused attacks for a second consecutive day, reducing immediate geopolitical tensions that had pushed crude prices above $100 per barrel.</p>
...
<p><a href="https://www.karmactive.com/category/energy/">Oil markets are highly sensitive to Middle East geopolitical developments</a> because the region contains global reserves critical to international energy supply. Disruptions to production or transportation through crucial shipping routes like the Strait of Hormuz directly impact global crude availability and prices. Beyond oil markets, the reduced military tension generated relief across global financial markets, with European and Asian stock markets rising following the news.</p>'

publish_post \
  "Oil Prices Plunge 8.7% as US-Iran Pause Military Hostilities - Brent Crude Falls to \$88" \
  "oil-prices-drop-us-iran-pause-2026" \
  "$CONTENT_16" \
  "[${CAT_ENERGY},${CAT_WORLD}]" \
  "[${TAG_OIL},${TAG_ENERGY},${TAG_MIDDLE_EAST},${TAG_IRAN}]" \
  "Oil Prices Plunge 8.7% as US-Iran Pause Military Hostilities" \
  "Oil prices drop 8.7% (Brent to \$88.36) and 7.5% (WTI to \$82.61) as US-Iran military pause announced. Diplomacy given space to resolve conflict. Market relief from geopolitical risk reduction." \
  "Oil prices US Iran"

# ══════════════════════════════════════════════════════════════════
# STORY 17: AI CHIP SELL-OFF
# ══════════════════════════════════════════════════════════════════
echo "[17/17] Publishing: AI Chip Sell-Off..."
CONTENT_17='<p>Major semiconductor manufacturers SK Hynix and Samsung Electronics experienced dramatic stock declines in July 2026, as investors reassessed expectations for artificial intelligence hardware demand amid concerns about slowing AI investment growth.</p>
...
<p>SK Hynix and Samsung are among the world&#8217;s largest suppliers of high-bandwidth memory chips essential for AI servers. <a href="https://www.karmactive.com/category/technology/">Their shares are particularly sensitive to shifts in expectations for spending by American technology giants</a> that constitute primary customers. The stock declines reflected investor recognition that extraordinary AI infrastructure spending may have created excess capacity relative to near-term demand growth, requiring adjustment in both market valuations and capital spending plans by semiconductor manufacturers.</p>'

publish_post \
  "AI Chip Stock Plunge: SK Hynix and Samsung Crash 14% Amid AI Demand Concerns" \
  "ai-chip-sell-off-sk-hynix-samsung-2026" \
  "$CONTENT_17" \
  "[${CAT_TECHNOLOGY},${CAT_AI}]" \
  "[${TAG_AI_TECHNOLOGY},${TAG_SEMICONDUCTOR},${TAG_STOCK_MARKET},${TAG_TECHNOLOGY}]" \
  "AI Chip Stock Plunge: SK Hynix and Samsung Crash 14% Amid AI Demand Concerns" \
  "SK Hynix plunges 14.6%, Samsung drops 9% on July 2. \$290B market cap loss. AI infrastructure demand concerns. Meta's internal cloud strategy, Chinese AI competition fuel investor pessimism." \
  "AI chip stock sell-off"

echo ""
echo "=============================="
echo " All 17 articles published!"
echo "=============================="
